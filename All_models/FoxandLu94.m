function [Y,T_run] = FoxandLu94(t, Ifunc, Area)
...
for i=2:nt
    ...
    Na_gates = Na_gates + dt*ANa(V0)*Na_gates+ sqrt(dt)*SNa(V0,Na_gates,NNa)*NaNoise(:,i-1);
    K_gates =  K_gates + dt*AK(V0) *K_gates + sqrt(dt)*SK(V0,K_gates)*KNoise(:,i-1);
